...
import { DollarSign, Hash, Clock, User } from "lucide-react";
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      ...
      <StatCard
        ...
        value={totalBookings}
        icon={<Hash size={20} />}
        description={`Confirmed: ₦${revenueByStatus.confirmed.toLocaleString()}`}
      />
      <StatCard
        title="Pending Revenue"
        value={`₦${revenueByStatus.pending.toLocaleString()}`}
        icon={<Clock size={20} />}
      ...
      <StatCard
        ...
        value={totalUsers}
        icon={<User size={20} />}
        description={`Avg per booking: ₦${avgBooking.toLocaleString()}`}
      />
    </div>
  );
};
